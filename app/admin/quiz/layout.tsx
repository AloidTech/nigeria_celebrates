...
import { Sliders, ClipboardList, Home, Shield } from 'lucide-react';

export default function AdminQuizLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className='min-h-screen bg-[#F5F5F0] text-[#1A1A1A] relative flex flex-col md:flex-row'>
            ...
            <aside className='fixed z-50 bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm md:max-w-none md:translate-x-0 md:left-6 md:top-1/2 md:-translate-y-1/2 md:bottom-auto md:w-20 bg-white/85 backdrop-blur-md rounded-2xl md:rounded-3xl border border-[#E5E5E5] shadow-xl p-3 md:py-6 flex flex-row md:flex-col items-center justify-around md:justify-center gap-2 md:gap-6 transition-all duration-300 hover:border-[#1A3C2E]/30 hover:shadow-2xl'>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className='text-[10px] md:hidden font-bold mt-1'>{item.name}</span>
                ...
                >
                    ...
                    <span className='text-[10px] md:hidden font-bold mt-1'>Home</span>
